...
@section('content')
    <div class="m-5 p-4">
        <h1 class="text-center text-2xl font-bold">La cocina de {{ $user->name }}</h1>
    </div>
...
<ul class="flex justify-center gap-1 text-gray-900 mt-6">
       ...
       @foreach ($recipes->getUrlRange(1, $recipes->lastPage()) as $page => $url)
           @if ($page == $recipes->currentPage())
               <li class="block size-8 rounded border border-yellow-500 bg-yellow-500 text-center p-1 text-sm font-medium text-white">
                   {{ $page }}
               </li>
           @else
               <li>
