...
<div class="bg-white rounded-md shadow w-full h-[500px] flex flex-col">
    <header class="p-4 flex justify-between items-center">
        ...
        <div class="flex items-center space-x-2"> <!-- Contenedor de la imagen y el nombre -->
            ...
            <a href="{{ route('recipe.user', $recipe->user) }}"
                class="uppercase font-bold text-md text-yellow-500 hover:underline">
                {{ $recipe->user->name }}
            </a>
        </div>
    </header>
    ...
    <div class="p-4">
        ...
        <p class="text-lg text-gray-600">Ultima vez actualizada:
            {{ \Carbon\Carbon::parse($recipe->updated_at)->diffForHumans() }}</p>
    </div>

    <footer
        class="m-auto p-4 bg-yellow-500 hover:bg-yellow-400 text-black py-2 px-6 rounded-full text-lg transition-all">
